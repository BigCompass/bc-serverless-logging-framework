...
it('initializes the log object correctly given the provided arguments', () => {
    const result = createLog_1.initLog(lager_1.lager.levels.info, [
        'test message 123',
        { testprop: 'test 123' }
    ]);
...
it('sets an error correctly', () => {
    ...
    const result = createLog_1.initLog(lager_1.lager.levels.error, [err]);
    expect(result).toEqual({
...
it('sets an error correctly when a custom error key is specified', () => {
    const err = new Error('test error');
    const result = createLog_1.initLog(lager_1.lager.levels.error, [err], 'errorObject');
    expect(result).toEqual({
        level: lager_1.lager.levels.error,
        errorObject: {
            name: err.name,
...
it('initializes a log containing only an object', () => {
    const result = createLog_1.initLog(lager_1.lager.levels.warn, [{ stuff: 'test123' }]);
    expect(result).toEqual({
...
it('initializes a log with message in arguments array after object', () => {
    const result = createLog_1.initLog(lager_1.lager.levels.warn, [{ stuff: 'test123' }, 'message']);
    expect(result).toEqual({
        level: lager_1.lager.levels.warn,
        message: 'message',
        stuff: 'test123'
