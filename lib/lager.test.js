...
it('creates a logger with default levels as functions', () => {
    const logger = lager_1.lager.create();
    for (let level of Object.values(lager_1.lager.levels)) {
        expect(typeof logger[level]).toBe('function');
    }
});
it('create a logger with specified levels when provided', () => {
    const logger = lager_1.lager.create({
        levels: ['test_level_1', 'test_level_2', lager_1.lager.levels.info]
    });
    expect(typeof logger.test_level_1).toBe('function');
    expect(typeof logger.test_level_2).toBe('function');
    expect(typeof logger[lager_1.lager.levels.info]).toBe('function');
});
it('logs with default props when specified', () => {
    ...
    const logger = lager_1.lager.create({
        ...
        transports: [
            {
                destination: lager_1.lager.destinations.consoleLog()
            }
        ]
    });
    // Run
    ...
    expect(JSON.parse(logData)).toMatchObject({
        level: lager_1.lager.levels.info,
...
it('utilizes transport level correctly', () => {
    ...
    const logger = lager_1.lager.create({
        transports: [
            {
                level: lager_1.lager.levels.warn,
                destination: lager_1.lager.destinations.consoleLog()
            }
        ]
    });
...
it('logs a warning if an invalid level is used for a transport', () => {
    ...
    expect(spies.console.warn).toHaveBeenCalledWith(`Invalid level detected in transport: dummy_level. This transport will run for all log levels. Valid levels: ${Object.values(lager_1.lager.levels)}`);
});
