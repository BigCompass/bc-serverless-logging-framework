...
it('initializes the log object correctly given the provided arguments', () => {
  const result = initLog(bcLogger.levels.info, [
    'test message 123',
    { testprop: 'test 123' }
  ])
...
it('sets an error correctly', () => {
  ...
  const result = initLog(bcLogger.levels.error, [err])
  expect(result).toEqual({
...
it('sets an error correctly when a custom error key is specified', () => {
  const err = new Error('test error')
  const result = initLog(bcLogger.levels.error, [err], 'errorObject')
  expect(result).toEqual({
    level: bcLogger.levels.error,
    errorObject: {
      name: err.name,
...
it('initializes a log containing only an object', () => {
  const result = initLog(bcLogger.levels.warn, [{ stuff: 'test123' }])
  expect(result).toEqual({
...
it('initializes a log with message in arguments array after object', () => {
  const result = initLog(bcLogger.levels.warn, [
    { stuff: 'test123' },
    'message'
  ])
  expect(result).toEqual({
